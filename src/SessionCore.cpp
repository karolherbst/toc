...
namespace TOC
{
	namespace core
	{
		...
		void
		Session_Core::
		handle_connect(const system::error_code& error,
		               tcp::resolver::iterator endpoint_iterator)
		{
			if (!error)
			{
		...
		void
		Session_Core::
		writeToAll(const ChatMessage& msg)
		{
			writeToAllExceptThis(msg,
			                     0);
		}
		...
		void
		Session_Core::
		writeToAllExceptThis(const ChatMessage& msg,
		                     Session_Core* _this)
		{
			for (std::set<Session_Core*>::iterator it = getSetOfOtherSessions().begin();
			     it != getSetOfOtherSessions().end();
		...
		Session_Core&
		Session_Core::
		operator<<(const ChatMessage& msg)
		{
			write(msg);
			return *this;
		}
		...
		void
		Session_Core::
		handleWrite(const system::error_code& error)
		{
			if (!error)
			{
		...
		close()
		{
			io_service_.post(bind(&Session_Core::do_close,
			                      this));
		}
